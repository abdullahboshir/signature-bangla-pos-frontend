  ...
  const handleSwitchUnit = (unitId: string) => {
    ...
    if (unitId === 'all') {
      ...
      if (effectiveCompanyId) {
        router.push(`${basePath}/dashboard?organization=${effectiveCompanyId}`);
      } else {
    ...
    if (unitId === 'add-new') {
      const url = effectiveCompanyId
        ? `${basePath}/business-units/new?organization=${effectiveCompanyId}`
    ...
    if (unit) {
      ...
      if (effectiveCompanyId) params.set('organization', effectiveCompanyId);
