 ...
 */
export const matchesRole = (source: string | string[] | null | undefined, target: string | string[]): boolean => {
 ...
 */
export const isSuperAdmin = (role: string | string[] | null | undefined): boolean => 
  matchesRole(role, USER_ROLES.SUPER_ADMIN);

export const isCompanyOwner = (role: string | string[] | null | undefined): boolean => 
  matchesRole(role, USER_ROLES.COMPANY_OWNER);

export const isBusinessAdmin = (role: string | string[] | null | undefined): boolean => 
  matchesRole(role, [USER_ROLES.ADMIN, 'business-admin']);

export const isManager = (role: string | string[] | null | undefined): boolean => 
  matchesRole(role, [USER_ROLES.MANAGER, USER_ROLES.OUTLET_MANAGER]);

/**
 * Common Role Groupings
 */
export const isPlatformLevel = (role: string | string[] | null | undefined): boolean => 
  matchesRole(role, [
...
export const isOperationalRole = (role: string | string[] | null | undefined): boolean => 
  matchesRole(role, [USER_ROLES.CASHIER, USER_ROLES.SALES_ASSOCIATE, USER_ROLES.STAFF]);
