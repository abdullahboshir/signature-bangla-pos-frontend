...
export default function CompanyAdminLayout({ children }: CompanyAdminLayoutProps) {
  ...
  const userRoles = useMemo(() => {
    ...
    console.log("Company Admin Layout - Raw user object:", user);
    ...
    console.log("Company Admin Layout - Extracted roles:", allRoles);
    return [...new Set(allRoles)]; // Remove duplicates
  }, [user]);

  // Company Admin layout allows: company-owner and platform-level users (super-admin can access everything)
  const hasAccess = useMemo(() => {
    const access = isCompanyOwner(userRoles) || isPlatformLevel(userRoles);
    console.log("Company Admin Layout - User Roles:", userRoles, "Has Access:", access);
    return access;
  }, [userRoles]);
  ...
  useEffect(() => {
    if (!isLoading && user) {
      console.log("Company Admin Layout - User loaded. Has access:", hasAccess, "Roles:", userRoles);
      if (!hasAccess) {
        console.warn("Company Admin Layout - User doesn't have access but allowing for now. Roles:", userRoles);
        // Temporarily disabled logout for debugging
        // toast.error("Access Denied: You don't have permission to access Company Admin area.");
        // logout();
      }
    }
