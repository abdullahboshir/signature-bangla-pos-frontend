...
export default function CompanyAdminDashboard() {
    ...
    const userCompanyId = user?.context?.primary?.organization?._id;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="col-span-1 lg:col-span-2 bg-card p-6 rounded-xl shadow-sm border border-border">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <button onClick={() => router.push(`/organization/business-units/new?organization=${filters.companyId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-primary bg-primary/5 border border-primary/20 rounded-lg hover:bg-primary/10 transition-colors">
                            <span>Add Business Unit</span> <Plus className="w-4 h-4" />
                        </button>
                        <button onClick={() => router.push(`/organization/user-management/business-users/add?organization=${filters.companyId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-lg hover:bg-emerald-100 transition-colors">
                            <span>Add Staff</span> <Plus className="w-4 h-4" />
                        </button>
                        <button onClick={() => router.push(`/organization/system/audit-logs?organization=${filters.companyId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-muted-foreground bg-muted/50 rounded-lg hover:bg-muted transition-colors">
