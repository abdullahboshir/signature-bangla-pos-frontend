...
import { PlatformMetricCards } from "@/components/modules/global-dashboard/PlatformMetricCards";
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="col-span-1 lg:col-span-2 bg-card p-6 rounded-xl shadow-sm border border-border">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {isPlatformView && (
                            <button onClick={() => router.push(`/global/companies/add?company=${filters.companyId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-primary bg-primary/5 border border-primary/20 rounded-lg hover:bg-primary/10 transition-colors">
                                <span>Add Company</span> <Plus className="w-4 h-4" />
                            </button>
                        )}
                        {!isPlatformView && (
                            <button onClick={() => router.push(`/global/business-units/new?company=${filters.companyId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-primary bg-primary/5 border border-primary/20 rounded-lg hover:bg-primary/10 transition-colors">
                                <span>Add Business Unit</span> <Plus className="w-4 h-4" />
                            </button>
                        )}
                        <button onClick={() => router.push(`/global/user-management/business-users/add?company=${filters.companyId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-lg hover:bg-emerald-100 transition-colors">
                            <span>Add staff</span> <Plus className="w-4 h-4" />
                        </button>
                        <button onClick={() => router.push(`/global/system/audit-logs?company=${filters.companyId || ''}`)} className="flex items-center justify-between p-3 text-sm font-medium text-muted-foreground bg-muted/50 rounded-lg hover:bg-muted transition-colors">
